...
set.seed(123)
pacman::p_load(cmdstanr, posterior, tidyverse, cowplot, loo, bayesplot, gridExtra, grid)
...
make_comp_table <- function(loo1, loo2, label1, label2, data_label) {
  comp  <- loo_compare(loo1, loo2)
  ratio <- abs(comp[2, "elpd_diff"]) / comp[2, "se_diff"]
  df <- data.frame(
    Model     = c(label1, label2),
    elpd_loo  = round(c(loo1$estimates["elpd_loo", "Estimate"],
                        loo2$estimates["elpd_loo", "Estimate"]), 1),
    elpd_diff = round(comp[, "elpd_diff"], 1),
    se_diff   = round(comp[, "se_diff"],   1),
    check.names = FALSE
  )
  list(df = df, ratio = ratio, label = data_label)
}

comp_rl     <- make_comp_table(loo_rl_on_rl, loo_biased_on_rl,
                               "RL model", "Biased model", "RL-generated data")
comp_biased <- make_comp_table(loo_rl_on_biased, loo_biased_on_biased,
...
# Page 1: Model comparison tables
for (comp in list(comp_rl, comp_biased)) {
  tbl <- tableGrob(comp$df, rows = NULL,
                   theme = ttheme_minimal(base_size = 12,
                     core    = list(fg_params = list(hjust = 0.5)),
                     colhead = list(fg_params = list(fontface = "bold"))))
  title_grob <- textGrob(
    paste0(comp$label, "   (|elpd_diff| / SE = ", round(comp$ratio, 1), ")"),
    gp = gpar(fontsize = 14, fontface = "bold")
  )
  grid.arrange(title_grob, tbl, ncol = 1, heights = c(0.15, 0.85))
}

# Page 3: Pointwise ELPD differences
print(plot_grid(p_pw_rl, p_pw_biased, ncol = 2, labels = c("A", "B"),
                label_size = 12))

# Page 4: Pareto k diagnostics
