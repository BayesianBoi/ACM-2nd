...
true_bias_per_trial <- rep(block_biases, each = n_trials / length(block_biases))

# 2. SIMULATION FUNCTION

simulate_rl_mp <- function(alpha, tau, n_trials, block_biases) {
  ...
  for (t in 1:n_trials) {
    ...
    opponent_choice[t] <- rbinom(1, 1, true_bias_per_trial[t])
...
true_bias_per_trial <- rep(block_biases, each = length(block_biases))
...
p_prior_trajectory <- ggplot(prior_trajectory_df, aes(x = trial)) +
  geom_point(aes(y = opponent, colour = "Opponent choices"),
             size = 1.2,
             alpha = 0.4) +
  geom_step(aes(y = true_bias, colour = "True bias"),
            linewidth = 1,
            linetype = "dashed") +
...
p_posterior_trajectory <- ggplot(post_trajectory_df, aes(x = trial)) +
  ...
  geom_step(aes(y = true_bias, colour = "True bias"),
            ...
            linetype = "dashed") +
  # geom_ribbon(aes(ymin = agent_lower, ymax = agent_upper), fill = "royalblue3", alpha = 0.25) +
  geom_line(aes(y = agent_mean, colour = "Agent belief (posterior)"),
            linewidth = 0.8) +
  scale_colour_manual(
    values = c(
      "Opponent choices" = "grey50",
...
plot_alpha <- ggplot() +
  ...
  annotate(
    ...
    x = sim_data$alpha,
    y = Inf,
    label = paste0("True (", sim_data$alpha, ")"),
    hjust = -0.1,
    vjust = 1.5,
    size = 3.5,
    colour = "#E84855"
  ...
  coord_cartesian(xlim = c(0, 1),
                  ylim = c(0, NA),
                  expand = c(0, 0)) +
  labs(title = "Prior–Posterior update: alpha", x     = "alpha", y     = "Density") +
  theme_cowplot()
...
plot_tau <- ggplot() +
  ...
  annotate(
    ...
    x = sim_data$tau,
    y = Inf,
    label = paste0("True (", sim_data$tau, ")"),
    hjust = -0.1,
    vjust = 1.5,
    size = 3.5,
    colour = "#E84855"
  ...
  coord_cartesian(xlim = c(0, 15),
                  ylim = c(0, NA),
                  expand = c(0, 0)) +
  labs(title = "Prior–Posterior update: tau", x     = "tau", y     = "Density") +
  theme_cowplot()
...
tau_grid <- c(0.1, 5, 10)
alpha_grid <- c(0.1, 0.5, 0.75)
n_trials_grid <- c(25, 50, 150)
...
recovery_long <- recovery_df %>%
  select(alpha_true, tau_true, alpha_est, tau_est, alpha_sd, tau_sd, n_trials) %>%
  pivot_longer(
...
prior_sense_power_scaling_plot <- powerscale_plot_dens(model_fit, variables =
                                                         c("alpha", "tau"))
...
parameter_recovery_mean_estimates_plot <- ggplot(recovery_long,
...
parameter_recovery_mean_estimates_plot_by_amount_trials <- ggplot(recovery_long,
                                                 aes(x = true_value, y = estimate, colour = parameter)) +
  geom_abline(
    slope = 1,
    intercept = 0,
  ...
  ylab("Estimated Value") +
  xlab("Ground Truth Value") +
  facet_wrap( ~ parameter + n_trials, scales = "free", labeller = as_labeller(function(name) ifelse(str_detect(name, "_est"), name, paste0(name, " trials")))) +
  scale_colour_brewer(palette = "Set2", guide = "none") +
  theme_cowplot() +
  labs(title = "Joint Parameter Recovery By Amount of Observations")

parameter_recovery_mean_estimates_plot_by_amount_trials
